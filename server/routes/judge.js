...
const auth = require('../middleware/auth'); // Ensure you have your auth middleware
...
// --- NEW XP UPDATE ROUTE ---
router.patch('/update-xp', auth, async (req, res) => {
    try {
        ...
        user.coins = (user.coins || 0) + Math.floor(points / 10); 
...
// --- UPDATED CODE RUNNER ROUTE ---
router.post('/run', auth, async (req, res) => { // Added auth middleware
    const { code, language, testCases, pointsEarned = 100 } = req.body; // Default points
    const filename = `quest_${Date.now()}`;
    let filePath = '';
    let command = '';
    let compileCommand = '';
    ...
    // ... (Your existing language logic for js, python, cpp, java remains exactly the same)
...
for i, tc in enumerate(test_cases):
    ...
    try {
        ...
        // --- AUTOMATIC XP INJECTION ON VICTORY ---
        let updatedStats = null;
        if (allPassed) {
            const user = await User.findById(req.user.id);
            user.xp += pointsEarned;
            user.coins += Math.floor(pointsEarned / 10);
            await user.save();
            updatedStats = { xp: user.xp, coins: user.coins };
        }
...
// --- UPDATED SQL JUDGE ROUTE ---
router.post('/run-sql', auth, (req, res) => { // Added auth middleware
    const { query, initScript, expectedOutput, pointsEarned = 50 } = req.body;
    ...
    db.serialize(() => {
        ...
        db.all(query, [], async (err, rows) => { // Added async for DB save
            db.close();
            if (err) {
